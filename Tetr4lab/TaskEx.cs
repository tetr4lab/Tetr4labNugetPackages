...
public static class TaskEx {
    ...
    /// <param name="limit">msec単位の制限</param>
    /// <param name="tick">刻み</param>
    public static async Task DelayWhile (Func<Task<bool>> predicate, int limit = 0, int tick = 0) {
        tick = (tick > 0) ? tick : Tick;
        if (limit <= 0) {
    ...
    /// <param name="limit">msec単位の制限</param>
    /// <param name="tick">刻み</param>
    public static async Task DelayUntil (Func<Task<bool>> predicate, int limit = 0, int tick = 0) {
        tick = (tick > 0) ? tick : Tick;
        if (limit <= 0) {
            while (!await predicate ()) {
                await Task.Delay (tick);
